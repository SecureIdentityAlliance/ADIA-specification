...
<h1>ACCOUNTABLE DIGITAL IDENTITY ARCHITECTURE SPECIFICATION</h1>
...
3.  A Credential Issuer is an organization that can collect, validate, package and then issue the information that the Service Provider needs.
...
3.  VC Issuer – A Credential Issuer connects to the ADI-Network via a VC Issuer role that can prepare and issue a VC based on validated Claims. The VC Issuer functions can be deployed by the Credential Issuer or by the VC Issuer Agent (or a combination of both).
...
Actors on an ADI-Network have an ADI-ROLE VC whose contents are specified in clause 6.7.4.
...
All entities within the architecture are represented by a Digital Address which is bound to one primary ADI-Network DID, and may additionally be represented by transaction DIDs as specified in clause 7.4.4. DIDdocs contain the public key of the DID.   DIDs digitally sign using their private key and can be verified using the DIDdoc public key.
...
```json
{
  ...
  "payload": {
    ...
    "credentialSubject": {
      ...
      "digital_address": "issuer_admin@ix_1",
      "role": "ISSUER",
      "id_doc": {
