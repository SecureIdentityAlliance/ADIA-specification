...
*Blocks: A-117, A-119, A-121, A-122*
...
The architect has proposed that ADI-ROLE Verifiable Credentials be removed and that each entity's role, entitlements and assurance ceilings be carried in its DID Document, signed by the enrolling authority as DID controller. The chain of trust is unchanged; the artefact carrying it changes. It simplifies enrollment and unifies key and authority lookup, at the cost of making DID resolution a dependency of every verification. A complete design is held by the editors and can be applied once the working group confirms the direction. It has deliberately not been applied to this draft.
...
| Ref | Severity | Clause or object | Issue | Work needed |
|---|---|---|---|---|
| B-203 | Critical | L1403 "Generate a PK pair" | Key-pair generation appears **after** `Create Digital Address` (L1104). The DID must bind to a public key that already exists | Reorder |
...
| Ref | Severity | Clause or object | Issue | Work needed |
|---|---|---|---|---|
| A-101 | Critical | B.1.2 / B.2.8 | Enrollment sends `role: "INTERCHANGE"`; role VC records `role: "IX"`. Same role, two tokens. AGD/ISSUER/SERVICE_PROVIDER match correctly across both. | Pick one token; align both *(awaiting D4)* |
| A-102 | Critical | B.2.9, B.2.10, B.2.11 | `id_doc.id` is the **Interchange's** DID (`f6e18f71…`), not the subject's. §3.20 and §8.5.1 require a DIDdoc to carry its own DID's key. Verification would use the wrong key | Set `id_doc.id` = `subject`, with the subject's key |
| A-103 | Critical | B.2.7 | `issuer` (`8c019421…`) ≠ `subject` (`71a39c8d…`) on a credential §9.1 requires to be self-signed | Make them identical |
| A-104 | Critical | B.2.7 | `subject` is `did:adi:71a39c8d…` but `id_doc.id` is `did:adi:71a39c8d…/region_1/` — same UUID, different path, trailing empty segment | Normalise per D-403 *(awaiting D2)* |
| A-105 | Critical | B.2.7 | AGD `authorized_to_issue: ["AGD-VC"]` — the root cannot issue Interchange credentials, so §9.2 is unexecutable and the network cannot bootstrap | Add the IX role-VC type *(awaiting D4)* |
| A-106 | Critical | B.2.8 | Interchange `authorized_to_issue: ["ADI-IX-VC"]` — an IX can only issue other IX credentials, contradicting §7.4.2, §9.3, §9.4, §9.5 | Set to Issuer + SP + User types *(awaiting D4)* |
| A-109 | Critical | B.2.11 | User VC `subject` ends `/issuer_1` — an Issuer path in a User credential — while `digital_address` is `USER_DA@IX_1`. Subject and DA disagree | Correct subject |
| A-110 | Critical | B.2.9 / B.2.10 | Both subjects share method-specific-id `3b576f82…`, differing only by path. Under DID Core, path is not part of the identifier: the Issuer and SP are **the same DID** | Distinct identifiers *(awaiting D2)* |
| A-112 | Critical | B.1.1, B.1.2, B.2.3, B.2.7–B.2.11 | `"type": "JWT"`. The JOSE header parameter is `typ` (RFC 7515 §4.1.9). Zero occurrences of `typ` or `kid` anywhere | Rename; add `kid` |
| A-122 | Critical | B.2.2 | Offer carries no `tx_code`, no `expires_in`, no single-use rule, no subject pre-binding — while §10.2.3 delivers it by QR | Add binding + TTL *(awaiting D5)* |
...
| A-113 | Major | B.2.2, B.2.3, B.2.7 | Three malformed UUIDs: `7560e8400-e79n-21d4-…` (9 hex in group 1, non-hex `n`) ×2, and `aa2f772-…` (7 in group 1) | Regenerate |
...
| A-116 | Major | B.2.2 | `subject: "did:adi:{subject_did}"` — placeholder syntax inconsistent with all other examples | Use a literal |
...
| A-118 | Major | B.3.1 | Metadata still branded `credential-issuer.example.com` (×4) while `authorization_servers` points at `users.adi.com` | De-brand |
| A-119 | Major | B.3.1 | `batch_credential_endpoint` and `deferred_credential_endpoint` were removed from later OIDC4VCI drafts. No draft version cited anywhere | Pin a draft; align *(awaiting D5)* |
...
| A-124 | Major | B.2.4 | VCDM **1.1** context (`2018/credentials/v1`), while §5.3 and the references cite VCDM 2.0. `vc_id` is not a VCDM member (should be `id`). `proof` is empty. No `credentialStatus`, no `credentialSchema` (zero occurrences document-wide) | Rebuild per D-802 *(awaiting D6)* |
| A-125 | Major | B.2.4 vs §8.2 | §7.2 states examples use JWT VC formatting; B.2.4 is plain JSON-LD | Reconcile *(awaiting D6)* |
| A-126 | Major | all JWT examples vs B.3.1 | `RS256` ×8 in examples, `ES256` ×3 in metadata. No mandatory-to-implement set, no algorithm-confusion prohibition, no minimum key size | Declare MTI set *(awaiting D7)* |
...
| D-405 | Major | L1020 "HIDA usage is optional" | HIDA is simultaneously mandatory ("Entities … are unique when they do not have matching HIDAs"; "The User HIDA **is** verified for uniqueness") and optional ("HIDA usage is optional"). Separately: no hash algorithm named, no salt/pepper/KDF/keyed-MAC, and **no canonicalization rule** — without normalization of case, diacritics, name order and date format the same person yields different HIDAs at different Interchanges and cross-region uniqueness silently fails | Decide; specify construction *(awaiting D9)* |
| D-406 | Major | L281/1005/1025 "pairwise" | §3.19 says entities *may* have pairwise DIDs; §8.5.3 says all DAs *have* them. Neither matters: **no flow or example uses one**. Every VC carries the primary DID, so every verifier gets the same global correlator | Reconcile with the protocol *(awaiting D12)* |
| D-408 | Major | L551/568 "Credential Provider" | Four incompatible scopes for "ADI Network Provider". §6.3 omits the Interchange entirely and calls the AGD an "Authoritative Domain Controller" (L522), a term used nowhere else. §6.2 says "Credential Provider" (L504 ×3) where §3.4 says "Credential Issuer" | Single taxonomy |
| D-411 | Major | L582/766 "The assurance level of" | The role-VC contents list appears twice and diverges: §6.3 says assurance of "the actor holding this role VC" (current), §7.7.4 says "the VC subject at the time of issuance" (point-in-time) | Delete one; cross-reference |
| D-412 | Major | L1266 "AGD Service Provider Directory" | L1040 says the SP Agent lists the SP in the AGD directory; the message sequence has the Interchange do it. No authorization model for directory writes | Resolve |
| D-413 | Major | L588/772 "encrypted by issuer" | Role VCs carry "PII (encrypted by issuer)" and those entities are published in the AGD directory. Encrypted to whom, under what key management, with what retention? Directory + PII + HIDA is a correlation database | Specify or remove |
...
| E-527 | Minor | L799/983 "red line" | Red-line reference points at Figure 4 in one place, the ADI Network figure in another | — |
| E-528 | Minor | L1035 "Figure 10." | Caption delimiter alternates: "Figure 3**:**" vs "Figure 10**.**"; some captions inline with the image, some on their own line | — |
